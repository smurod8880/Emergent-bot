...
class TradingCore:
    ...
    async def _initialize_ai_predictor(self):
        ...
        try:
            ...
            try:
                ...
                logger.info("🆕 Используется новая модель")
    ...
    async def start_trading(self):
        ...
        try:
            ...
            if self.is_running:
                logger.warning("⚠️ Торговля уже запущена")
                return
        ...
        try:
            ...
            while self.is_running:
                try:
                    ...
                    await self._send_performance_report(stats)
    ...
    async def _send_performance_report(self, stats: Dict[str, Any]):
...
🗄 База данных:
• Сигналов сегодня: {db_stats.get('total_signals', 0)}
• Успешных: {db_stats.get('successful_signals', 0)}
• Точность: {db_stats.get('accuracy', 0):.1f}%

🔌 WebSocket:
• Статус: {'🟢 Активен' if ws_stats.get('is_running', False) else '🔴 Неактивен'}
• Соединений: {ws_stats.get('connections_count', 0)}
• Пар: {ws_stats.get('pairs_count', 0)}

🤖 AI Модель:
• Предсказаний: {ai_stats.get('total_predictions', 0)}
• Точность: {ai_stats.get('average_accuracy', 0):.1f}%
• Ошибка: {ai_stats.get('average_error', 0):.3f}

⚙️ Ядро:
• Циклов анализа: {core_stats.get('total_analysis_cycles', 0)}
• Успешных: {core_stats.get('successful_analysis_cycles', 0)}
• Успешность: {core_stats.get('success_rate', 0):.1f}%
• Время работы: {core_stats.get('uptime', 0) // 3600:.0f}ч {(core_stats.get('uptime', 0) % 3600) // 60:.0f}м
            """
