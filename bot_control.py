    ...
    async def stop_trading(self):
        ...
        try:
            ...
            await self.telegram.send_message(
                ...
                f"📈 Сигналов отправлено: {self.signals_sent_today}\n"
...
await self._wait_next_cycle(cycle_start)
            ...
            for signal in signals:
                try:
                    ...
                    if success:
                        ...
                        performance_stats['daily_signals'] += 1
                        performance_stats['hourly_signals'] += 1
                        
                        logger.info(f"✅ Сигнал отправлен: {signal['pair']} {signal['timeframe']}")
    ...
    async def _validate_signal(self, signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Проверка точности
            if signal['accuracy'] < STRATEGY_CONFIG['target_accuracy'] * 100:
                logger.debug(f"📊 Сигнал не прошел проверку точности: {signal['accuracy']}%")
                return False
                
            # Проверка интервала между сигналами
            if self.last_signal_time:
                time_diff = (datetime.now() - self.last_signal_time).total_seconds()
                if time_diff < SAFETY_LIMITS['min_signal_interval']:
    ...
    def _check_signal_limits(self) -> bool:
        ...
        try:
            ...
            if self.signals_sent_hour >= SAFETY_LIMITS['max_signals_per_hour']:
...
# Проверка дневного лимита
    ...
    async def _wait_next_cycle(self, cycle_start: datetime):
        ...
        try:
            ...
            wait_time = max(0, STRATEGY_CONFIG['update_interval'] - cycle_duration)
